...
class Link:
    ...
    def __str__(self):
        return f"Type: {self.link_type}, URL: {self.url}, Valid: {GREEN if self.valid else RED}{self.valid}{NC}"
    def is_valid(self):
        return self.valid
...
# Create objet ArgumentParser
parser = argparse.ArgumentParser(description='Scripts with arguments example')

# Add arguments with modifiers
parser.add_argument('-n', '--name', help='Filename start scan')
parser.add_argument('-f', '--folder', help='Folder start scan')

# Analize arguments from command line
args = parser.parse_args()

if args.name and not args.name.isspace():
    filename= args.name

if args.folder and not args.folder.isspace():
